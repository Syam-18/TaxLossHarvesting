...
export const HoldingsRow = ({ holding, isSelected, onToggle }: HoldingsRowProps) => {
  ...
  return (
    <tr className={`border-b border-white/5 transition-colors ${isSelected ? 'bg-[#1e293b]/70' : 'hover:bg-white/[0.02]'}`}>
      <td className="p-1 md:p-2 pl-4 md:pl-6 align-middle">
      ...
      <td className="p-1 md:p-2 align-middle">
        <div className="flex items-end gap-3">
          <img src={holding.logo} alt={holding.coinName} className="w-8 h-8 rounded-full object-cover self-center" />
          <div className="flex flex-col">
            <span className="font-medium text-white/95 text-sm md:text-[14px]">{holding.coinName}</span>
            <span className="text-xs md:text-[12px] text-gray-400">{holding.coin}</span>
          </div>
        </div>
      </td>
      ...
      <td className="p-1 md:p-2 pr-6 text-right align-middle">
        {isSelected ? (
